...
template <size_t MAP_WIDTH, size_t MAP_HEIGHT, float TILE_SIZE>
entity_move_return_code try_move_entity_with_deltaSpeed_change_on_collision(Position &position, SpeedVector &speedVector, HitBoxRadius &hitBoxRadius, Map<MAP_WIDTH, MAP_HEIGHT, TILE_SIZE> &map, SpeedVector change, DebugConfiguration &debugConfiguration, const float frameTime) {
