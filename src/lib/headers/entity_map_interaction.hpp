...
std::array<TileType, 4> get_map_collision_tiles(Position &position, HitBoxRadius &hitBoxRadius, Map<MAP_WIDTH, MAP_HEIGHT> &map, DebugConfiguration &debugCfg) {
    ...
    for (size_t y : std::views::iota(playerMapCoordinatesRange.first.second, playerMapCoordinatesRange.second.second)) {
        for (size_t x : std::views::iota(playerMapCoordinatesRange.first.first, playerMapCoordinatesRange.second.first)) {
            ...
            TileType tileType = CheckCollisionCircleRec(position, hitBoxRadius.value(), tile) ? map.get_tile_type(x, y) : TileType(tile_type::EMPTY);
